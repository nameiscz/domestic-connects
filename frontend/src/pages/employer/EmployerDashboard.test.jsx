...
const mockData = ({ jobs = JOBS, workers = WORKERS } = {}) => {
  axiosInstance.get.mockImplementation((url) => {
    ...
    if (match && PERF[match[1]]) {
      return Promise.resolve({ data: PERF[match[1]] });
    }
...
describe('EmployerDashboard', () => {
  ...
  it('renders the job stat cards', async () => {
    ...
    expect(screen.getByText('Workers hired')).toBeInTheDocument();
    expect(screen.getByText('Avg. wage / day')).toBeInTheDocument();
    expect(screen.getByText('Avg. worker rating')).toBeInTheDocument();
    // Two assigned postings for employer 5.
