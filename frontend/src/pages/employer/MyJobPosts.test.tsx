...
const REPORTS: Record<number, unknown> = {
  11: {
    workerId: 11,
    reviewCount: 2,
    averageRating: 4.5,
...
const findRowFor = async (title: string): Promise<HTMLElement> => {
...
describe('MyJobPosts', () => {
  beforeEach(() => {
    ...
    axiosInstance.get.mockImplementation((url: string) => {
      ...
      if (perfMatch) {
        return Promise.resolve({ data: REPORTS[Number(perfMatch[1])] ?? null });
      }
  ...
  describe('assign worker flow', () => {
    it('only offers Assign for OPEN jobs', async () => {
      ...
      const assignedRow = screen.getByText('Cook').closest('tr') as HTMLElement; // status ASSIGNED
