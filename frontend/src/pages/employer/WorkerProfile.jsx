...
export default function WorkerProfile() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const pool = Array.isArray(workersRes.value?.data?.data)
        ? workersRes.value.data.data
        : [];
      const found = pool.find((w) => String(w.id) === String(workerId));
      if (!found) {
      ...
      setError(err.response?.data?.message || 'Unable to load the worker profile.');
    } finally {
      setLoading(false);
    }
