...
describe('WorkerProfile (employer view)', () => {
  ...
  it('assigns the worker to an open job via the reviewed endpoint', async () => {
    ...
    await user.selectOptions(within(assignCard as HTMLElement).getByLabelText('Job to assign'), '1');
    await user.click(within(assignCard as HTMLElement).getByRole('button', { name: /assign this worker/i }));
