...
beforeAll(() => {
  ...
  Element.prototype.getBoundingClientRect = function (): DOMRect {
    ...
    if (tag === 'text' || tag === 'tspan' || this.id === 'recharts_measurement_span') {
      return { ...rect, width: text.length * 7 + 4, height: 14 } as DOMRect;
    }
    ...
    if (classes.includes('recharts-wrapper') || classes.includes('recharts-responsive-container')) {
      return { ...rect, width: 640, height: 300 } as DOMRect;
    }
    if (this instanceof HTMLElement) {
      return { ...rect, width: 640, height: 24 } as DOMRect;
    }
    return rect;
  };
});
...
describe('AdminAnalytics', () => {
  ...
  it('lets the admin pick a previous month and refetches analytics for it', async () => {
    ...
    const selector = screen.getByLabelText(/analytics month/i) as HTMLSelectElement;
