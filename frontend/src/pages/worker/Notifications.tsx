...
const TYPE_META: Record<NotificationType, { variant: string; label: string }> = {
...
const typeMeta = (type: NotificationType): { variant: string; label: string } =>
  TYPE_META[type] || { variant: 'secondary', label: type };
...
export default function Notifications() {
  ...
  const [notifications, setNotifications] = useState<NotificationLog[] | null>(null); // null = not loaded
  ...
  const handleMarkRead = async (notification: NotificationLog) => {
    ...
    try {
      ...
      pushToast(
        (err as { response?: { data?: { message?: string } } })?.response?.data
          ?.message || 'Unable to mark as read. Please try again.',
        'danger'
      );
    } finally {
      setMarkingId(null);
