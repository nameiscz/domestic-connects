...
const renderPage = (currentUser: typeof EMPLOYER_USER | null = EMPLOYER_USER) => {
...
const selectWorker = async (user: ReturnType<typeof userEvent.setup>, workerId: number) => {
...
describe('ManageReviews', () => {
  ...
  it('opens the edit modal prefilled, PUTs rating/remarks and refreshes the report', async () => {
    ...
    expect((within(modal).getByLabelText('Remarks') as HTMLTextAreaElement).value).toBe('Excellent work, very punctual.');
