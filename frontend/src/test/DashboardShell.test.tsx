...
// render the real route structure from App.tsx (ProtectedRoute → dashboard →
...
// main.tsx mounts the app inside <React.StrictMode>, so render through it too
// (double-mounting must not duplicate the DOM — exactly what these tests guard).
const renderShell = (ui: ReactNode) => render(<StrictMode>{ui}</StrictMode>);
...
describe('Dashboard shell renders nested routes exactly once', () => {
  ...
  it('worker shell: /worker/performance appears once with the shell intact', async () => {
    ...
    expect(screen.getAllByRole('heading', { name: 'Performance' })).toHaveLength(1);
  ...
  it('parent route shows the overview only — the nested page must not render', async () => {
    ...
    expect(
      screen.getAllByRole('heading', { name: /upcoming schedule/i })
    ).toHaveLength(1);
