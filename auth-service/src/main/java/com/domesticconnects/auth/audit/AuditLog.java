 ...
 * Note: public auth endpoints (login, register, refresh) are whitelisted by
 * the gateway, which strips identity headers there — so calls to those
 * endpoints are recorded with actor {@code "anonymous"}. Authenticated admin
 * endpoints (e.g. user management) do carry the headers.
 * {@code oldValue} / {@code newValue} hold compact JSON summaries of the
 * pre-/post-change state (truncated by the aspect).
 * <p>
